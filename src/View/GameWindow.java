...
public class GameWindow extends JFrame {
    ...
    private void initGUI() {
        ...
        rollDiceButton = createImageButton("resources/dicesRolling.png",
                "Roll the dice to move", e -> onRollDice());
        // single-player: enabled; multiplayer: start disabled until TURN: arrives
        rollDiceButton.setEnabled(netClient == null);

        bottom.add(makeButtonCard(rollDiceButton));
        bottom.add(makeButtonCard(createImageButton("resources/goBack.png","Leave Game", e->onLeave())));
        bottom.add(makeButtonCard(createImageButton("resources/playerInfoWindow.png",
                "Show player info", e->onPlayerInfo())));
        bottom.add(makeButtonCard(createImageButton("resources/undo.png",
        ...
        for(int i=0; i<40; i++){
            boardView.getBoardPanel()
                    .setManualTileImage(i, new ImageIcon("resources/tiles/"+i+".png").getImage());
        }
            ...
            netClient.setListener(new NetworkClient.GameMessageListener(){
                @Override public void onFullState(GameState state){
                    SwingUtilities.invokeLater(() -> {
                        ...
                        boardView.refreshBoard();
                        boardView.drawPlayerTokens(_controller.getAllPlayers());
                        // re-enable roll if it's your turn
                ...
                @Override public void onState(String outcome){
                    // only active player sees buy/auction dialog
    ...
    private JButton createImageButton(String path, String tip, ActionListener l){
